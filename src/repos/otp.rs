use crate::{config::db::pool, entities::otp::{Otp, OtpCreate, OtpMedia, OtpType, OtpUpdate}};
use chrono::Utc;
...
impl OtpRepo {
    pub async fn get_by_key_username(key: &str, username: &str) -> Result<Otp, String> {
        ...
        ", key, username).fetch_one(&db).await;
        match res {
            Ok(rec) =>
                Ok(Otp {
                    id: rec.id,
...
impl Crud for OtpRepo {
    ...
    async fn get_by_id(id: uuid::Uuid) -> Result<Self::ReadModel, String> {
        let db = pool().await.clone();
        let res = sqlx::query!("SELECT * FROM otps WHERE id=$1", id).fetch_one(&db).await;
        match res {
            Ok(rec) =>
                Ok(Otp {
                    id: rec.id,
    ...
    async fn create(data: Self::CreateModel) -> Result<Self::ReadModel, String> {
        ...
        ",
            ...
            OtpMedia::to_int(data.media)).fetch_one(&db).await;
    ...
    async fn update(data: Self::UpdateModel) -> Result<Self::ReadModel, String> {
        let db = pool().await.clone();
        let res = sqlx::query!("UPDATE otps SET
            updated_at = $1,
            key = $2,
            is_used = $3,
            user_id = $4,
    ...
    async fn delete(id: uuid::Uuid) -> Result<uuid::Uuid, String> {
        let db = pool().await.clone();
        let res = sqlx::query!("DELETE FROM otps WHERE id = $1", id).execute(&db).await;
        match res {
            Ok(_) => Ok(id),
