use crate::{config::db::pool, entities::user::{User, UserCreate, UserUnsafe, UserUpdate, UserUpdateSafe}};
...
impl UserRepo {
    pub async fn get_by_username(username: &str) -> Result<UserUnsafe, String> {
        ...
        let res = sqlx::query!("SELECT * FROM users WHERE username=$1", username).fetch_one(&db).await;
        match res {
            Ok(rec) =>
                Ok(UserUnsafe {
                    id: rec.id,
    ...
    pub async fn update_safe(data: UserUpdateSafe) -> Result<User, String> {
        let db = pool().await.clone();
        let res = sqlx::query!("UPDATE users SET
            updated_at = $1,
            username = $2,
            phone = $3,
            email = $4,
...
impl Crud for UserRepo {
    ...
    async fn get_by_id(id: uuid::Uuid) -> Result<Self::ReadModel, String> {
        let db = pool().await.clone();
        let res = sqlx::query!("SELECT * FROM users WHERE id = $1", id).fetch_one(&db).await;
        match res {
            Ok(data) =>
                Ok(User {
                    id: data.id,
    ...
    async fn create(data: Self::CreateModel) -> Result<Self::ReadModel, String> {
        ...
        ",
            ...
            data.phone_verified,
            data.email_verified,
            data.is_admin).fetch_one(&db).await;
    ...
    async fn delete(id: uuid::Uuid) -> Result<uuid::Uuid, String> {
        let db = pool().await.clone();
        let res = sqlx::query!("DELETE FROM users WHERE id = $1", id).execute(&db).await;
        match res {
            Ok(_) => Ok(id),
